...
## Правило 19. `type: "null"` — это значение, а не отсутствие

JSON `null` приходит по проводу как значение. Sury-схема для `unit` ждёт
`undefined`, поэтому поле `{"type": "null"}`, напечатанное как `unit`, не
парсит ровно то единственное значение, которое спека разрешает.

Лоуеринг: `Schema.Null` → `IR.Nullable(Primitive(PUnit))` + аннотация `SNull`,
то есть `@s.null Nullable.t<unit>` в ReScript, `unit option` в OCaml,
`Option<()>` в Rust. Найдено тестом «парсит собственный пример» — компиляция
такое не ловит, потому что `unit` компилируется прекрасно.
