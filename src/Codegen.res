...
let rec generateType = (schema: Schema.schemaType): string => {
  ...
  | Nullable(inner) => `null<${generateType(inner)}>`
...
and generateRecord = (fields: array<Schema.field>): string => {
  if Array.length(fields) == 0 {
    ...
    let fieldStrs = fields->Array.map(field => {
      ...
      // Add @s.null attribute on the type (not field) for Nullable types
...
let generateModule = (schemas: array<OpenAPIParser.namedSchema>): string => {
  ...
  // Add null type with genType import for proper T | null TypeScript output
  let nullTypeImport = `@genType.import(("./Null.gen.ts", "t"))
type null<'a> = option<'a>

`
  // Add module alias required by sury-ppx
  nullTypeImport ++ "module S = Sury\n\n" ++ typeDefs
}
...
/** Generate Null.gen.ts shim content for @genType */
let generateNullShim = (): string => {
