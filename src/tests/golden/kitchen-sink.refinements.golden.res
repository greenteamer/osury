...
@schema
type refined = {
  id: @s.matches(S.uuid) string,
  email: @s.matches(S.email) string,
  site: @s.matches(S.uri) string,
  created_at: @s.matches(S.isoDateTime) string,
...
type getV1WidgetsParams = {
  limit: @s.with(S.gte(_, 1)) @s.with(S.lte(_, 100)) int,
